...
from backtest_engine.common.enums import FIFOOperationType, FIFOSide, AtomicActionType
...
class OpenShortStrategy(FIFOResolveStrategy):

    """
        Strategy for opening short positions in fifo.

        Responsibilities:
          - Create a new FIFOOperation of type OPEN with side SHORT.
          - Use the action's quantity and timestamp, and the provided execution price.
          - Append the new position to the SHORT side of the fifo queue.

        Invariants enforced:
          - Action type must be OPEN_SHORT.
    ...
    EXPECTED_TYPE: ClassVar[AtomicActionType] = AtomicActionType.OPEN_SHORT
    REQUIRED_GUARDS = [
