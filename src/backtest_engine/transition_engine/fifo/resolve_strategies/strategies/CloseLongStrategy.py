...
from backtest_engine.common.enums import FIFOSide, FIFOOperationType, AtomicActionType
from backtest_engine.common.types import FIFOPosition, FIFOOperation
from backtest_engine.transition_engine.fifo.guards_strategies.expectations import ActionPriceExpectation, \
    ActionQuantityExpectation, ActionTypeExpectation, ActionTimestampExpectation, FIFOCapacityExpectation
from backtest_engine.transition_engine.fifo.guards_strategies.factory import SafeGuardFactory
from backtest_engine.transition_engine.fifo.guards_strategies.interface import ResolveContext
from backtest_engine.transition_engine.fifo.resolve_strategies.registry import register_fifo_resolve_strategy

from backtest_engine.transition_engine.fifo.resolve_strategies.interface import FIFOResolveStrategy

@register_fifo_resolve_strategy(AtomicActionType.CLOSE_LONG)
class CloseLongStrategy(FIFOResolveStrategy):
    """
        Strategy for closing-long positions in fifo.

        Responsibilities:
          - Match the action's quantity against existing active LONG positions.
          - Generate one or more FIFOOperations of type CLOSE with side LONG.
          - Link each close operation to its originating fifo position.

        Invariants enforced:
          - Action type must be CLOSE_LONG.
    ...
    EXPECTED_TYPE: ClassVar[AtomicActionType] = AtomicActionType.CLOSE_LONG
    REQUIRED_GUARDS = [
    ...
    def resolve(
        ...
        fifo = fifo_queues[FIFOSide.LONG]
        ...
        if remaining_qty > 0:
            raise ValueError(
                f"[{self.STRATEGY_NAME}] Quantity to close ({action.quantity}) exceeds available fifo positions ({action.quantity - remaining_qty})."
            )
